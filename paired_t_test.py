...
pkl_path = r'E:\eyetracking\nwb files\isolated_eye_events.pkl'
...
def run_fixation_stats(path):
    ...
    for pid, sessions in master_dict.items():
        # Grabbing the 'duration' field directly as it from 'behavior' module
        enc_durations = [e['duration'] for e in sessions.get('Encoding', []) if e['event'] == 'fixation']
        rec_durations = [e['duration'] for e in sessions.get('Recognition', []) if e['event'] == 'fixation']

        # Ensure we have a pair for the t-test
